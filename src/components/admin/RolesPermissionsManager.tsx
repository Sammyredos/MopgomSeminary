...
export function RolesPermissionsManager() {
  ...
  const canEditRoles = ['Super Admin', 'School Administrator', 'Admin'].includes(currentUser?.role?.name || '')
  const canViewRoles = ['Super Admin', 'School Administrator', 'Admin'].includes(currentUser?.role?.name || '')
    ...
    <div className="space-y-6">
      ...
      <Dialog open={showCreateRole} onOpenChange={setShowCreateRole}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle className="font-apercu-bold text-lg text-gray-900">Create New Role</DialogTitle>
          ...
          <DialogFooter className="p-4 sm:p-6 border-t flex justify-end space-x-3 flex-shrink-0">
      ...
      <Dialog open={showEditRole} onOpenChange={setShowEditRole}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <div className="flex items-center justify-between">
              <DialogTitle className="font-apercu-bold text-lg text-gray-900">Edit Role</DialogTitle>
              {editingRole && (
      ...
      <Dialog open={!!editingRole && !showEditRole} onOpenChange={(open) => !open && setEditingRole(null)}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <div className="flex items-center justify-between">
              <DialogTitle className="font-apercu-bold text-lg text-gray-900">{editingRole?.name}</DialogTitle>
              {editingRole && (
                <Badge className={editingRole.isSystem ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}>
                  {editingRole.isSystem ? 'System Role' : 'Custom Role'}
                </Badge>
          ...
          <DialogFooter className="p-4 sm:p-6 border-t flex justify-end flex-shrink-0">
      ...
      <Dialog open={showDeleteConfirm.show} onOpenChange={(open) => !open && setShowDeleteConfirm({show: false, role: null})}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <div className="flex items-center mb-4">
              ...
              <div>
                <DialogTitle className="font-apercu-bold text-lg text-gray-900">Delete Role</DialogTitle>
                <DialogDescription className="font-apercu-regular text-sm text-gray-600">
          ...
          <DialogFooter className="flex justify-end space-x-3">
            ...
            <Button
              onClick={() => showDeleteConfirm.role && deleteRole(showDeleteConfirm.role.id, showDeleteConfirm.role.name)}
