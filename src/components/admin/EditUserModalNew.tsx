...
export function EditUserModal({ isOpen, onClose, onUserUpdated, user, roles }: EditUserModalProps) {
  ...
  const assignableRoles = currentUser ? filterAssignableRoles(roles, currentUser.role.name) : roles

  useEffect(() => {
    if (user) {
      // Parse the name if individual fields are not available
      let surname = user.surname || ''
      let firstname = user.firstname || ''
      let lastname = user.lastname || ''
      
      // If individual name fields are empty but we have a full name, try to parse it
      if (!surname && !firstname && !lastname && user.name) {
        ...
        if (nameParts.length >= 3) {
          lastname = nameParts.slice(2).join(' ') // Join remaining parts as lastname
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData: any = {
        name: `${formData.surname} ${formData.firstname} ${formData.lastname}`.trim(),
      ...
      <Card className="w-full max-w-md bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-4 sm:p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* First Row: Surname and First Name */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Surname */}
              <div>
                <label htmlFor="surname" className="block font-apercu-medium text-sm text-gray-700 mb-2">
                  Surname *
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    readOnly={user?.role?.name === 'Student'}
                    className={`w-full pl-10 pr-4 py-3 border rounded-xl font-apercu-regular transition-all duration-200 ${
                      user?.role?.name === 'Student' 
                        ? 'bg-gray-100 border-gray-200 cursor-not-allowed text-gray-600' 
                        : fieldErrors.surname 
                          ? 'border-red-300 bg-red-50 focus:outline-none focus:ring-2 focus:ring-indigo-500' 
                          : 'border-gray-300 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500'
                    }`}
                    placeholder="Enter surname"
                  />
                </div>
                {fieldErrors.surname && (
                  <p className="mt-1 text-sm text-red-600 font-apercu-medium">{fieldErrors.surname}</p>
                )}
              </div>

              {/* First Name */}
              <div>
                <label htmlFor="firstname" className="block font-apercu-medium text-sm text-gray-700 mb-2">
                  First Name *
                </label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                ...
                {fieldErrors.firstname && (
                  <p className="mt-1 text-sm text-red-600 font-apercu-medium">{fieldErrors.firstname}</p>
                )}
              </div>
            </div>

            {/* Second Row: Last Name and Role */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Last Name */}
              <div>
                <label htmlFor="lastname" className="block font-apercu-medium text-sm text-gray-700 mb-2">
                  Last Name *
                </label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                ...
                {fieldErrors.lastname && (
                  <p className="mt-1 text-sm text-red-600 font-apercu-medium">{fieldErrors.lastname}</p>
                )}
              </div>
