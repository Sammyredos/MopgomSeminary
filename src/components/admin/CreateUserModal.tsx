...
export function CreateUserModal({ isOpen, onClose, onUserCreated, roles }: CreateUserModalProps) {
  ...
  const assignableRoles = currentUser ? filterAssignableRoles(roles, currentUser.role.name) : []
    ...
    try {
      const response = await fetch('/api/admin/users/create', {
        ...
        body: JSON.stringify({
          ...
          roleName: assignableRoles.find(r => r.id === formData.roleId)?.name,
          userType: 'admin' // Default to admin type for now
        }),
      })
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              {formData.roleId && (
                <div className="mt-2">
                  {(() => {
                    const selectedRole = assignableRoles.find(r => r.id === formData.roleId)
                    return selectedRole ? (
