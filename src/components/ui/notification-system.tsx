...
function NotificationItem({ id, type, title, message, duration = 5000, action, onRemove }: NotificationItemProps) {
  ...
  const getStyles = () => {
    switch (type) {
      case 'success':
        return 'bg-green-50 border-l-4 border-green-500 shadow-lg shadow-green-500/10'
      case 'error':
      ...
      <div className={cn(
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3 flex-1">
            <p className={`font-apercu-medium text-sm ${
              type === 'success' ? 'text-green-800' : 'text-gray-900'
            }`}>
              {title}
            </p>
            {message && (
              <p className={`font-apercu-regular text-sm mt-1 ${
                type === 'success' ? 'text-green-700' : 'text-gray-600'
              }`}>
                {message}
              </p>
            )}
...
export function useNotificationSystem() {
  ...
  return {
    ...
    toast: (options: { title: string; description?: string; variant?: 'default' | 'destructive' }) => {
      ...
      addNotification({ type, title: options.title, message: options.description })
    }
  }
}
