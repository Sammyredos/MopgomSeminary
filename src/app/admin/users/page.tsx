...
export default function UsersPage() {
  ...
  // Add instant search state for client-side filtering
  const [instantSearchTerm, setInstantSearchTerm] = useState('')

  // Add instant filtering logic similar to communications page
  const filteredUsers = useMemo(() => {
    if (!instantSearchTerm && !filterRole) return users.filter(user => user.id !== currentUser?.id)
    ...
    return users.filter(user => {
      ...
      // Apply role filter
      if (filterRole && user.role?.id !== filterRole) return false

      // Apply instant search filter
      if (instantSearchTerm) {
        const searchLower = instantSearchTerm.toLowerCase()
  ...
  const fetchUsers = useCallback(async (page = 1, isRefresh = false) => {
    try {
      ...
      if (response.ok) {
        ...
        const fetchedUsers = (data.users || []).filter((user: User) => !recentlyDeletedIds.has(user.id))
  ...
  // Check permissions - Allow Super Admin, Admin, and Lecturer roles
          ...
          <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
            ...
            <div className="relative lg:w-96 w-full">
              <Search className={`absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 transition-colors ${
                instantSearchTerm ? 'text-indigo-500' : 'text-gray-400'
              }`} />
              <input
                type="text"
                ...
                className={`w-full pl-10 pr-10 py-2.5 border rounded-lg font-apercu-regular focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm transition-all duration-200 hover:border-indigo-300 ${
                  instantSearchTerm ? 'border-indigo-300 bg-indigo-50/30' : 'border-gray-300'
                }`}
              />
              {instantSearchTerm && (
                <button
          ...
          <div className="pt-3 border-t border-gray-200">
            <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2">
              <p className="font-apercu-regular text-sm text-gray-600">
                {instantSearchTerm ? (
                  <>
                    Found <span className="font-apercu-medium text-indigo-600">{formatNumber(filteredUsers.length)}</span> users matching &quot;<span className="font-apercu-medium text-indigo-600">{instantSearchTerm}</span>&quot;
                  </>
                ) : (
                  <>
        ...
        {filteredUsers.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="font-apercu-medium text-gray-500">
              {instantSearchTerm || filterRole ? 'No users match your search' : 'No users found'}
            </p>
