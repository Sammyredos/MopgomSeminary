...
export async function getSessionTimeout() {
  try {
    // Read from systemConfig table (where admin settings are saved)
    ...
    // Fallback to old method if not found in systemConfig
...
export async function getLoginAttemptsLimit() {
  try {
    ...
    // Fallback to old method if not found in systemConfig
    return await getSetting('security', 'maxLoginAttempts', 5)
  } catch (error) {
    console.error('Error getting login attempts limit:', error)
    return 5
  }
}
...
export async function getLockoutDuration() {
  try {
    ...
    console.error('Error getting lockout duration:', error)
    return 30
  }
}
...
export async function getSystemName() {
  return await getSetting('branding', 'systemName', 'Mopgom TS')
}
