...
export class SecurityValidator {
  ...
  /**
   * Enhanced password validation that uses admin settings
   */
  static async validatePasswordWithSettings(password: string): Promise<{ valid: boolean; errors: string[] }> {
    const { settingsService } = await import('./settings-service')
    const errors: string[] = []
    
    // Get password requirements from admin settings
    const minLength = await settingsService.getSetting('passwordMinLength', 8)
    const requireStrong = await settingsService.getSetting('requireStrongPassword', true)
    ...
    // Check strong password requirements if enabled
    if (requireStrong) {
      if (!/[A-Z]/.test(password)) {
        errors.push('Password must contain at least one uppercase letter')
      }
   ...
   */
  static async getPasswordRequirements(): Promise<{ minLength: number; requireStrong: boolean; requirements: string[] }> {
    const { settingsService } = await import('./settings-service')
